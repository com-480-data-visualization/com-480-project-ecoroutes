...
We selected a diverse set of 90 cities across Europe, guided by a principle to include at least one city from each country, alongside with others noted for their size and popularity. The city list was derived from the [Figure 1. Regions of Europe](https://open.lib.umn.edu/worldgeography/chapter/2-3-regions-of-western-europe/). For each possible city pair, we extracted the travel times and environmental impacts (carbon dioxide emissions and energy consumption)  across different modes of transportation (cars, trains, and planes). Moreover, we detailed the 'products' of each travel connection, which denotes the required changes in flights or trains necessary to complete each journey.
